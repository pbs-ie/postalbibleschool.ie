export default function AnchorNavLink({ href, isResponsive = false, newTab = false, isDropdown = false, children }: { href: string, isResponsive?: boolean, newTab?: boolean, isDropdown?: boolean, children?: React.ReactNode }) {
    const getClassNames = () => {
        if (isResponsive) {
            return "w-full flex items-start rounded px-2 pr-4 py-2 text-base font-medium transition duration-150 ease-in-out hover:text-gray-800 hover:bg-gray-300 focus:text-gray-800 focus:bg-gray-50 dark:hover:text-gray-100 dark:hover:bg-gray-700";
        } else if (isDropdown) {
            return "inline-flex items-center whitespace-nowrap bg-clip-padding py-3 px-6 w-full text-sm font-medium leading-5  focus:bg-slate-200 transition duration-150 ease-in-out";
        } else {
