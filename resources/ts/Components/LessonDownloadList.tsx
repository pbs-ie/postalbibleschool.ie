...
import { gleanersSeriesNames, goingDeeperSeriesNames, monthNames, type SeriesName, seriesNames } from "@/constants";
...
export default function LessonDownloadList({ tagClass = "bg-bibletime-pink", tagCode = "level0", isWideScreen = false, type = "bibletime" }: LessonDownloadList) {
    ...
    return (
        <div className={`grid ${gridCols} gap-1`}>
            {
                getSeries().map((seriesElement) => (
                    <div key={seriesElement.code} className="flex flex-col gap-0.5">
                        <div className={`text-center h-fit w-full text-gray-50 ${getActiveTagClass(seriesElement)} p-2 mb-2 rounded`}>{seriesElement.name}</div>
                        {
                            monthNames.map((month, index) => (
                                <LessonDownloadButton key={seriesElement.code + month} downloadLink={getDownloadLink(seriesElement.code, tagCode, index, type)} infoClass={getActiveTagClass(seriesElement)} title={`${seriesElement.code}${(index + 1)}${type !== "gleaners" && isWideScreen ? ` -  ${month}` : ''}`} />
                            ))
                        }
                    </div>
                ))
