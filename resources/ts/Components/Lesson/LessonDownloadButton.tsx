export default function LessonDownloadButton({ title, infoText, infoClass = "bg-pbsblue", downloadLink = "" }: { title: string, infoText: string | null, infoClass: string, downloadLink: string | undefined }) {
    ...
    const getButtonColorClass = () => {
        if (isLinkEnabled())
            return `hover:${infoClass} bg-stone-200 text-slate-800 hover:text-slate-50`;
        else
    ...
    return (
        <>
            {infoText &&
                <a className="block" href={downloadLink} target="_blank"><button disabled={!isLinkEnabled()} className={`flex flex-row items-center h-fit w-full ${getButtonColorClass()} rounded-md`}>
            ...
            {!infoText &&
                <a className="block" href={downloadLink} target="_blank"><button disabled={!isLinkEnabled()} className={`flex flex-row items-center h-fit w-full ${getButtonColorClass()} py-2 rounded`}>
