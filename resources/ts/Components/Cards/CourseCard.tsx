...
export default function CourseCard({ heading, image, description, type = "bibletime", scrollTo, buttonText }: CourseContent) {
    ...
    return (
        <div className="grid content-between h-full grid-cols-1 pt-4 rounded-md lg:p-6 min-w-min bg-slate-100 drop-shadow-lg">
            <Heading3>{heading}</Heading3>
            <img className="h-auto mx-auto w-52" src={image} alt={`${heading}`} />
            <div className="p-4 pb-0 text-sm leading-tight">{description}</div>

            {scrollTo ?
                <><div className="flex flex-wrap items-start content-start justify-center flex-auto gap-px mb-2">
                    {
                        levelGroup.map((element) => (
                            <TagGroupPill key={element.tagName} addClass={element.tagClass}>{element.tagName}</TagGroupPill>
                        ))
                    }
                </div>
